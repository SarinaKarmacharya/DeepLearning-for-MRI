...
def read_and_decode(filename_queue):
    ...
    image.set_shape([67108864])
    image_re = tf.reshape(image, (512,512,256))

    image_re = tf.cast(image_re, tf.float32) * (1. / 1024)
    label = tf.decode_raw(features['label_raw'], tf.uint8)
    label.set_shape([8388608])
    label_re = tf.reshape(label, [256,256, 128])
...
def inference(images):
    ...
    images_re=tf.reshape(images, [-1,512,512,256,1])
...
#upsampling 4
    ...
    with tf.name_scope('conv16'):
         W_conv16=tf.Variable(tf.truncated_normal([3,3,3,16, 2], stddev=0.1,
         ...
         W_uptrans4=tf.Variable(tf.truncated_normal([2,2,2,2,2],stddev=0.1,
                                                dtype=tf.float32), name='W_uptrans4')
         
         up_conv_final=tf.nn.conv3d_transpose(relu19_op, W_uptrans4, [100, 256, 256, 128, 2], strides=[1, 1, 1, 1, 1], padding='VALID', name='up_conv_final')

         return up_conv_final
